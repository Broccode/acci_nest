...
export class RedisCacheService implements CacheService {
  ...
  private buildKey(key: string, options?: CacheOptions): string {
    const tenant = options?.tenant || (this.tenantContext.hasTenant() ? this.tenantContext.getCurrentTenant() : null);
  ...
  async get<T>(key: string, options?: CacheOptions): Promise<T | null> {
    ...
    try {
      ...
      this.logger.error(`Failed to get from cache: ${fullKey}`, error instanceof Error ? error.stack : String(error));
  ...
  async set<T>(key: string, value: T, options?: CacheOptions): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to store in cache: ${fullKey}`, error instanceof Error ? error.stack : String(error));
  ...
  async delete(key: string, options?: CacheOptions): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to delete from cache: ${fullKey}`, error instanceof Error ? error.stack : String(error));
