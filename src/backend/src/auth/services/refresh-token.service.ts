...
export class RefreshTokenService {
   ...
   * @param user The user associated with the token (needed for generating new token)
   * @returns Object containing the new token and validity status
   */
  async rotateRefreshToken(
    ...
    if (!tokenData || tokenData.userId !== user.id) {
      // If token is invalid or doesn't belong to the user, invalidate but don't generate new
