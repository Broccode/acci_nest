  ...
  async start(): Promise<TestingModule> {
    ...
    while (retries < maxRetries) {
      try {
        ...
        console.error(
          `Failed to start PostgreSQL container (attempt ${retries + 1}/${maxRetries}):`,
          error
        );
    ...
    throw new Error(
      `Failed to start PostgreSQL container after ${maxRetries} attempts: ${lastError?.message}`
    );
  }
  ...
  private async createTestTenants(): Promise<void> {
    ...
    try {
      ...
      if (tenantEntityClass) {
        ...
        console.log(
          'Registered entities:',
          this.options.entities.map((e) => (typeof e === 'function' ? e.name : e))
        );
      }
    } catch (error) {
