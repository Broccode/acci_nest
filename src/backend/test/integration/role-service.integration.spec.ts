...
describe('RoleService Integration Tests', () => {
  ...
  it('should assign permissions to a role', async () => {
    ...
    const roleWithPermissions = await em.findOne(
      Role,
      { id: role.id, tenantId: testTenant1.id },
  ...
  it('should remove permissions from a role', async () => {
    ...
    await roleService.removePermissions(role.id, [writePermission.id], testTenant1.id);
  ...
  it('should handle system roles properly', async () => {
    ...
    await roleService.assignPermissions(systemRole.id, [adminPermission.id], testTenant1.id);

    // Verify permissions were assigned
    const roleWithPermissions = await em.findOne(
      Role,
      { id: systemRole.id, tenantId: testTenant1.id },
      { populate: ['permissions'] }
    );
    expect(roleWithPermissions).toBeDefined();
    ...
    await expect(
      roleService.updateRole(systemRole.id, { name: 'ModifiedSystemAdmin' }, testTenant1.id)
    ).rejects.toThrow('System roles cannot be modified');

    // Try to remove permission - should fail
    await expect(
      roleService.removePermissions(systemRole.id, [adminPermission.id], testTenant1.id)
