...
describe('UserService Integration Tests', () => {
  ...
  it('should create a user with proper tenant isolation', async () => {
    ...
    const foundUser = await em.findOne(User, { email: 'test@example.com', tenantId: testTenant1.id });
    expect(foundUser).toBeDefined();
    expect(foundUser?.id).toBe(user.id);
    
    // Verify user isolation (should not be visible in other tenant)
    environment.setTenantId(testTenant2.id);
    const notFoundUser = await em.findOne(User, { email: 'test@example.com', tenantId: testTenant2.id });
  ...
  it('should assign roles to a user', async () => {
    ...
    const userWithRoles = await em.findOne(User, { id: user.id, tenantId: testTenant1.id }, { populate: ['roles'] });
