...
export class Migration20250404153810 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "permission" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "resource" varchar(255) not null, "action" varchar(255) not null, "conditions" jsonb null, constraint "permission_pkey" primary key ("id"));`
    );
    this.addSql(
      `create index "permission_resource_action_index" on "permission" ("resource", "action");`
    );

    this.addSql(
      `create table "role" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" varchar(255) not null, "description" varchar(255) null, "tenant_reference_id" uuid not null, "tenant_id" varchar(255) not null, "is_system" boolean not null default false, constraint "role_pkey" primary key ("id"));`
    );
    ...
    this.addSql(
      `alter table "role" add constraint "role_tenant_reference_id_foreign" foreign key ("tenant_reference_id") references "tenants" ("id") on update cascade;`
    );
    ...
    this.addSql(
      `alter table "user" add constraint "user_tenant_reference_id_foreign" foreign key ("tenant_reference_id") references "tenants" ("id") on update cascade;`
    );
